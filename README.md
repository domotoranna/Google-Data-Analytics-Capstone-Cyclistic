...
The dataset includes multiple years of data. Analyzing a single year ensures consistency, possibly one that is devided into quarters rather than months, so I chose to work with data from 2019.
...
- Each quarterly file contains approximately 360,000 rows, which exceeds the capacity of Excel sheets. Cleaning and analyzing such a large dataset in Excel would be time-consuming and impractical.
...
A clear example is February, where customers make up only about 2.5% of total users, with the remaining majority being subscribers.
...
3. Weekend Passes: As weekend trips are a common use case among casual riders, Cyclistic could offer weekend passes at a lower price point compared to the annual membership. This would appeal to customers who use the service for leisure or occasional trips, potentially increasing weekend usage and converting more casual riders into regular users.
